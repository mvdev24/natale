...
// Stato scelte finali
const tripState = {
  city: "la scegliamo insieme",
  timing: "quando saremo tutti liberi",
  contact: "(aggiungete un contatto)",
};
...
function answerStep(step, part, correct, selection) {
  ...
  if (part === 1) {
    if (correct) {
      if (step === 3) {
        tripState.city = selection || tripState.city;
      }
  ...
  if (part === 2) {
    if (correct) {
      if (step === 3) {
        tripState.timing = selection || tripState.timing;
      }
...
// Carica l'immagine di Babbo Natale
const santaImage = new Image();
santaImage.src = "assets/santa-claus.png?v=20251225";
...
class SantaFace {
  ...
  reset(initial = false) {
    ...
    this.y = initial ? Math.random() * canvas.height : -10 - Math.random() * 60;
    this.size = Math.random() * 20 + 20; // dimensioni più piccole (20-40px)
  ...
  draw() {
    if (!santaImage.complete) return; // Aspetta che l'immagine sia caricata
    ...
    ctx.drawImage(santaImage, -halfSize, -halfSize, this.size, this.size);
...
const santas = [];
for (let i = 0; i < 25; i++) {
  santas.push(new SantaFace());
}

function animateSantas() {
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  santas.forEach((f) => {
    f.update();
    f.draw();
  });
  requestAnimationFrame(animateSantas);
}
...
function finalizeTrip() {
  ...
  errorEl.classList.add("hidden");
  tripState.contact = contactValue;
  renderFinal();
  goTo("final");
}

function renderFinal() {
  const destEl = document.getElementById("finalDest");
  const whenEl = document.getElementById("finalWhen");
  const contactEl = document.getElementById("finalContact");
  if (destEl) destEl.textContent = tripState.city;
  if (whenEl) whenEl.textContent = tripState.timing;
  if (contactEl) contactEl.textContent = tripState.contact;
}
